...
The site is designed to look clean and minimal, similar to Apple's style. Blog posts are loaded directly from the repository's `blogs` directory using generated `blog-index.json` and `blog-index.js` files, so new content can be added without modifying application code.
...
3. Run `node generate-blog-index.js` to rebuild `blog-index.json` and `blog-index.js`.
